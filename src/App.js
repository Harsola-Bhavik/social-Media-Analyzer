...
class LangflowClient {
    ...
    async post(endpoint, body, headers = {"Content-Type": "application/json"}) {
        ...
        try {
            ...
            if (!response.ok) {
              console.error(`HTTP Error: ${response.status} ${response.statusText} - ${JSON.stringify(json)}`);
                throw new Error(`${response.status} ${response.statusText} - ${JSON.stringify(json)}`);
            }
            return json;
        } catch (error) {
    ...
    async runFlow(flowIdOrName, langflowId, inputValue, inputType = 'chat', outputType = 'chat', tweaks = {}, stream = false, onUpdate, onClose, onError) {
        try {
            ...
            if (stream && initResponse && initResponse.outputs && initResponse.outputs[0]?.outputs?.[0]?.artifacts?.stream_url) {
              ...
              console.log(`Streaming from: ${streamUrl}`);
                this.handleStream(streamUrl, onUpdate, onClose, onError);
                return initResponse; // Return immediately when stream is opened
            }
            return initResponse;
            ...
            console.error('Error running flow:', error);
            onError('Error initiating session');
            throw error; // Re-throw the error to be caught in App
        }
    }
}
...
function App() {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState('');
    const [activeTab, setActiveTab] = useState('chat'); // Set default active tab to 'chat'
    ...
    const handleSubmit = async () => {
        ...
        try {
            setIsLoading(true);
            setError('');
            setMessages(prev => [...prev, { type: 'user', text: inputValue }]); // Add user message immediately
            const userMessage = inputValue;
            setInputValue('');
              ...
              if (flowOutput.outputs && flowOutput.outputs[0]) {
                  const componentOutput = flowOutput.outputs[0];
                   console.log('Component output:', JSON.stringify(componentOutput, null, 2));
                    ...
                    if (componentOutput.outputs?.message?.message?.text) {
                      botResponse = componentOutput.outputs.message.message.text;
                    } else if (componentOutput.outputs?.message?.text) {
                        botResponse = componentOutput.outputs.message.text;
                    } else if (componentOutput.message?.text) {
                        botResponse = componentOutput.message.text;
                    } else if (componentOutput.text) {
                        botResponse = componentOutput.text;
                  }
                ...
                console.error('Invalid response structure: No outputs[0] found within flowOutput.outputs');
                setError('Invalid response from server (no output)');
              }

           } else {
              console.error('Invalid response structure: No outputs[0] found within response');
              setError('Invalid response from server');
           }
        } catch (error) {
            console.error('Error details:', error);
